...
- Принимать обычную ссылку на товар Wildberries, текст с одной ссылкой или список
  ссылок в личном чате Telegram. Названия и цены из окружающего текста бот не использует.
- Импортировать товары из переданной пользователем shared basket Wildberries.
  Ссылка на корзину может находиться внутри обычного сообщения.
...
- `/start` — главное меню.
- `/tracked` — мои товары.
- `/add` — добавить товар по обычной ссылке, тексту из Copy/Share или списку ссылок.
...
Они определены в `src/main/resources/application.properties`:
...
Отправьте боту `/start`, затем обычную ссылку Wildberries, текст из Copy/Share
или сообщение со списком ссылок:
