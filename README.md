...
- `PriceContext` хранит региональные параметры;
- `UserPricePreferences` хранит процент оценки WB Кошелька.

Новый пользователь создаётся с Moscow, `dest=1259570991`, `spp=30` и wallet
discount 3%. UI изменения региона и скидки в текущем MVP отсутствует.
...
### Один регион по умолчанию

Модель поддерживает `PriceContext`, но Telegram UX выбора региона ещё не
реализован. Новый пользователь получает Moscow.
