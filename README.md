...
> пользователя может зависеть от региона, способа оплаты, персональных скидок,
...
- Выбирать один из поддерживаемых регионов цен; по умолчанию используется Москва.
...
3. `UserProfileService` находит пользователя или создаёт профиль с настройками
   по умолчанию. Выбранный регион профиля преобразуется в `PriceContext` для
...
- `PriceContext` quote совпадает с текущим регионом пользователя.
...
Хранит фиксированный каталог поддерживаемых городов и выбранный регион пользователя.
...
Новый и существующий профиль по умолчанию использует Москву. Изменить регион можно
только при отсутствии активных подписок: существующие `WatchTarget` и их история
не переносятся и не смешиваются с ценами другого региона. Product URL и shared basket
используют текущий регион профиля, а scheduler продолжает использовать `PriceContext`,
...
- `region_code` связывает профиль с фиксированным каталогом регионов;
- `UserPricePreferences` хранит процент оценки WB Кошелька.

Новый пользователь получает регион Москва и wallet discount 3%. Регион можно
...
Именно поэтому два пользователя с одинаковым товаром, вариантом и региональным
...
| Таблица | Назначение |
|---|---|
| `user_profiles` | Telegram identity, регион и пользовательская wallet preference |
...
- «Регион цен» — выбрать один из фиксированных поддерживаемых городов.
...
- регион нового пользователя: Москва; каталог регионов и технические WB destination
...
способ оплаты и часть региональных условий. Wallet price — только estimate.

### Фиксированный каталог регионов

MVP не ищет города или ПВЗ автоматически и не принимает произвольный регион.
...
9. Проверить доступ VPS к Telegram и Wildberries из выбранного региона.
